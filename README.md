...
├─ 启动.bat                  # exe 版一键启动（自动生成 .env、检查是否填好）
├─ start.bat / start.sh      # 源码版启动脚本
...
└─ scripts/
   ...
   ├─ check.js               # 离线自检 141 项（不需要 QQ、不需要 key）
   ├─ integration.js         # 集成测试 84 项（假 NapCat + 假 DeepSeek）
...
`--help` 会把 `.env` / `persona.md` / 图库的**绝对路径**打出来，遇到「改了没生效」先跑它看一眼。
...
# ① 离线自检：141 项，不需要 QQ、不需要 key、不联网
npm test

# ② 集成测试：84 项，用本地假 NapCat + 假 DeepSeek 跑通整条链路
...
| 现象 | 原因 |
|---|---|
| 双击 exe 一闪就没了 | 在项目目录按住 Shift 右键 → 「在此处打开 PowerShell」→ 敲 `.\qqbot.exe` 看报错 |
